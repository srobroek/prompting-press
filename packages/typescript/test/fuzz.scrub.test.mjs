...
`);
  ...
  const secretArb = fc.string({ minLength: 8, maxLength: 40 }).map(
    (suffix) => `sk-LIVE-${suffix}`,
  );
  ...
  fc.assert(
    fc.property(secretArb, (secret) => {
      ...
      try {
        ...
        threw = true;
        assertSecretAbsent(err, secret);
        assert.ok(err instanceof PromptRenderError || err instanceof PromptingPressError,
...
`);
  ...
  assert.throws(
    ...
    (err) => {
      ...
      assert.ok(err.message.includes("forbidden prefix"),
