...
`);
  ...
  assert.throws(
    ...
    (err) => {
      ...
      assert.deepEqual(err.errors.map((row) => row.code), ["render"]);
...
// ── 10. UnknownPromptError no longer applies (no registry lookup) ─────────────────────────

test("UnknownPromptError is exported and is a PromptingPressError subclass", () => {
  // Post-reshape there is no registry, so UnknownPromptError is not thrown during render.
  // It may still appear if the addon surfaces it in some edge case; the class must exist.
