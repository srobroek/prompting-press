...
const SYS_PREAMBLE = makePrompt({ name: "sys_preamble", role: "system", body: "You are helpful.", variables: {} });
...
test("the fromMessages path preserves order, roles, and per-entry text (SC-008)", () => {
  ...
  assert.deepEqual(messages.map((m) => m.text), ["You are helpful.", "Hi Bo"]);
});
...
test("invalid vars at append throw PromptValidationError and store nothing (no partial)", () => {
  ...
  assert.deepEqual(comp.resolve().map((m) => m.text), ["Hi ok"]);
});
...
test("the first invalid entry in fromMessages throws and yields no Composition (no partial)", () => {
  ...
  assert.deepEqual(good.resolve().map((m) => m.text), ["Hi ok"]);
});
...
test("a variant entry field selects the named variant arm", () => {
  ...
  assert.deepEqual(viaFactory.resolve().map((m) => m.text), ["Good day, Di"]);

  const viaAppend = new Composition();
  viaAppend.append({ prompt: WITH_VARIANT, schema: Named, data: { name: "Di" }, variant: "formal" });
...
test("no variant field defaults to the reserved default arm", () => {
  ...
  assert.deepEqual(comp.resolve().map((m) => m.text), ["Hi Eli"]);
});
...
test("a mixed system + two user composition resolves to 3 ordered messages (SC-008)", () => {
  ...
  assert.deepEqual(messages.map((m) => m.text), ["You are helpful.", "Hi Ada", "Bye Bo"]);
});
...
test("static (no-schema) entries are marshaled directly (Q4)", () => {
  ...
  assert.deepEqual(comp.resolve().map((m) => m.text), ["You are helpful.", "Hi Zed"]);
});
