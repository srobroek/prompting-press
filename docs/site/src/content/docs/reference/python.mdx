...
The opt-in guard-expansion config, surfaced to Python and **plumbed through** to the kernel. A 1:1 mirror of the kernel's [`prompting_press_core::GuardConfig`]. This pyclass is
**config only**; it carries no logic. The kernel owns guard *expansion*;
...
it once via the constructor. ## Advisory override `advisory` replaces the fixed default wording returned in `RenderResult.guard`. The override
