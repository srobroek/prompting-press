...
```ts
new Prompt(shape: PromptDefinition, validators?: ValidatorMap, _internal?: InternalCtorArg): Prompt
```
...
The output of [`check`]: an ordered, read-only list of [`Finding`]s. Empty ⇒ the lint passed. The Node mirror of the consumer's CheckReport (; ). Surfaced **1:1** — the binding adds nothing and interprets nothing. A `` class with read-only accessors; a report is produced by [`check`], never constructed from JS.
...
One resolved message in a composition's output: a role-tagged rendered string. The Node mirror of the consumer's `Message` (). `role` is the prompt definition's role stringified (`"system"` / `"user"` / `"assistant"`); `text` is that prompt rendered with the entry's own validated value. A `` so it crosses as a plain JS object `&#123; role, text &#125;`; a message is produced by [`Composition::resolve`], never constructed from JS.
