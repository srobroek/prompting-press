...
The opt-in guard-expansion config, accepted from JS as a plain object and **plumbed through** to the kernel. A 1:1 mirror of the kernel's GuardConfig — `enabled` only. This is **config only**; it carries no logic. The kernel owns guard *expansion*; the binding only marshals `enabled` across the boundary and surfaces whatever [`RenderResult::guard`] the kernel populates. Accepted from JS as a plain TS object `&#123; enabled &#125;`.
