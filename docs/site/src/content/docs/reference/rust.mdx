...
The prompt declares `field` as `trusted: false` but configures no guard for it (the trust/guard advisory — FR-018; the `metadata.guard` convention, module docs). The only advisory class surfaced by `Prompt::check()`.
