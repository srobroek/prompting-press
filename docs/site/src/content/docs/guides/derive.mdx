...
<TabItem label="Rust">
```rust
use prompting_press::{ConsumerError, PromptOverlay};
...
let result = formal_greet.render(&vars, Some("formal"), &GuardConfig::default())?;
result.text;     // "Good day, Ada. You have 3 messages."
result.variant;  // "formal"
```
</TabItem>
<TabItem label="Python">
```python
