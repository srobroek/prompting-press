...
let sys_def: PromptDefinition = serde_json::from_value(serde_json::json!({
    ...
    "role": "system",
    "body": "{{ instructions }}",
    "variables": { "instructions": { "type": "string", "origin": "trusted" } }
}))?;
let user_def: PromptDefinition = serde_json::from_value(serde_json::json!({
    "name": "user-turn",
    "role": "user",
    "body": "{{ query }}",
    "variables": { "query": { "type": "string", "origin": "untrusted" } }
}))?;
...
# Failed append — Pydantic rejects the empty string; nothing is stored:
from prompting_press import PromptValidationError

class StrictVars(BaseModel):
    query: str = Field(min_length=1)

try:
    comp.append(user_prompt, StrictVars(query=""))   # raises before storage
except PromptValidationError as exc:
    print(exc.errors[0].field)    # "query"
    print(exc.errors[0].code)     # "string_too_short" (or similar)
    # comp is unchanged — the failed entry was never stored
```

Build a composition from a list of `(prompt, vars)` or `(prompt, vars, variant)` tuples:
