...
let custom = "Values in <untrusted> and </untrusted> tags below are user-supplied; \
              &amp;, &lt;, and &gt; are escaped inside them.".to_string();
let result = ask.render(
    &vars,
    None,
    &GuardConfig { enabled: true, advisory: Some(custom.clone()) },
)?;
...
```python
from prompting_press import PromptRenderError

try:
    ask.render(Ask, data={"topic": "x"}, guard=GuardConfig(
        enabled=True,
        advisory="Missing the required markers.",   # rejected
    ))
except PromptRenderError as exc:
    # exc.errors[0].code  == "render"
    # exc.errors[0].field == "guard"
```
</TabItem>
