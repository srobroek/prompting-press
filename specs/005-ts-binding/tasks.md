...
- [ ] T026 Build the distributable package + fresh-env import (SC-009): `mise exec -- pnpm -C packages/typescript build` → the platform `.node` + ESM entry; in a clean dir `pnpm pack` + install the tarball and `import('prompting-press')` + run a one-line render. Confirm ESM-only resolution works on Node 20+ (scaffold `engines.node: ">=20"`).
