...
This repository is built to work with Spec Kit Memory Hub and OpenAI Codex (and Codex-compatible agents).
...
2. **Proactive Check**: Before planning or tasking, you MUST prepare context using the best available path:
   - MCP: call `speckit_memory_refresh_cache`, `speckit_memory_search`, `speckit_memory_synthesize`, and `speckit_memory_synthesize_docs` if your Codex environment exposes MCP tools
...
> **Note for Codex environments**: MCP tool availability depends on your Codex client configuration. If MCP tools are not available, use the markdown-first fallback workflow.
