...
Date: 2026-06-30
Driver: spec 015 (Guard delimiting redesign) clarify session.
Change: the opt-in guard, when enabled, now DELIMITS untrusted values directly in the
  rendered body (fixed `<untrusted>…</untrusted>` tags, entity-escaping `<`/`>`/`&` inside
  the value) instead of leaving the body byte-identical and only naming fields in a separate
  advisory string. This redefines the guard-on body-invariant established by spec 002
  (FR-022/FR-023/SC-005) in a BACKWARD-INCOMPATIBLE way → MAJOR.
  - Principle III (Minimal Boundary): clarified — inserting structural delimiter markers
    around an untrusted value's rendered interpolation is IN-boundary (still "rendered text
    + provenance"; no I/O, no LLM, no request assembly). The non-mutation doctrine is
    narrowed: VALUE CONTENT is never altered (no sanitize/strip/semantic change — spec 002
    FR-025 preserved); only structural markers are added, and entity-escaping is reversible
    structure, not content mutation.
  - Principle V (Provenance): clarified — `render_hash = SHA256(rendered output)` is preserved
    in FORM; the rendered output now depends on whether the guard is enabled. Guard-on ⇒ hash
    of the delimited body; guard-off ⇒ hash of the plain body. Both deterministic; the caller
    records which mode produced a hash.
  - Coupled API/schema changes ratified by this amendment (detail in spec 015): the per-
    variable `origin` enum (trusted|untrusted|external) collapses to a `trusted` BOOLEAN, and
    the caller-supplied custom guard template (spec 002 FR-024) is removed (fixed advisory that
    references the markers). These are breaking schema/API changes landing with spec 015.
Migration: callers who enabled the guard and relied on the body being byte-identical to a plain
  render MUST update — the guard-on body now contains delimiter markup and `render_hash` differs.
  The guard-OFF body invariant is fully preserved. Recorded in DECISIONS.md.
Templates checked:
...
- The opt-in guard, when enabled, MAY insert **structural delimiter markers** around an
  untrusted value's rendered interpolation (e.g. `<untrusted>…</untrusted>`, with `<`/`>`/`&`
  inside the value entity-escaped). This is **in-boundary**: the library is still producing
  *rendered text + provenance*, performing no I/O, no LLM call, and no request-body assembly. The
  non-mutation doctrine is narrowed accordingly — the **value's content is never altered** (no
...
- The `render_hash = SHA256(rendered output)` form is preserved, but the **rendered output now
  depends on whether the guard is enabled** (amended for spec 015, v2.0.0): guard-on ⇒ the hash of
...
**Version**: 2.0.0 | **Ratified**: 2026-06-25 | **Last Amended**: 2026-06-30
